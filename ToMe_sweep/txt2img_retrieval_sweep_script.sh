...
BASEBASE_DIR=/fsx-checkpoints/yejinlee/sweep/txt_to_img/retrieval/${TASK}.${MODEL_NAME}/bs${BATCH_SIZE}.n_retrieved_doc${NRETRIEVED_DOCS}

mkdir -p $BASEBASE_DIR
...
if [ -d "$EXP_DIR/images" ] ; then
    ...
    FSD=/fsx-onellm/mingdachen/onellm-eval-data-tok python scripts/cm3v2/gen_images.py --exp_name $EXP_NAME \
        ...
        --exp_dir $BASE_DIR --cfg $CFG --temp $TEMP --topp $TOPP --seed $SEED --num-cfg-samples $BATCH_SIZE --generate --n-retrieved-docs $NRETRIEVED_DOCS > $BASE_DIR/time.txt
        ...
        do
            ...
            do
                ...
                if [ -d "$EXP_DIR/images" ] ; then
                    ...
                    FSD=/fsx-onellm/mingdachen/onellm-eval-data-tok python scripts/cm3v2/gen_images.py --exp_name $EXP_NAME \
                        --model-name $MODEL_NAME --batch-size $NUM_SAMPLE --tasks $TASK \
                        --exp_dir $BASE_DIR --cfg $CFG --temp $TEMP --topp $TOPP --seed $SEED --num-cfg-samples $BATCH_SIZE --generate --n-retrieved-docs $NRETRIEVED_DOCS \
