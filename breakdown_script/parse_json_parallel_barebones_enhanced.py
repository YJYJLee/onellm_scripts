...
def parse_file(file_path, save_folder_path=None, plot_graph=True):
        ...
        for idx,l in enumerate(jsonString):
            ...
            if "cat" in l and l["cat"]=="kernel":
                ...
                if "nccl" in l["name"]:
                    ...
                    if found==False:
                        print("ERROR2")
                        for ii in range(0,len(jsonString)): # should find the launch kernel closest to this event
                            if jsonString[ii]["name"] == "cudaLaunchKernel": # if there is a launch kernel there
                                gpu_launch_kernels.append(jsonString[ii]) # From(cpu)
                                gpu_kernels.append(l) # To(gpu kernel) # append this to the GPU kernel
                                found=True
    ...
    decoding_step_start_time = [gpu_kernels_dict[gpu_launch_kernels[0]['args']['External id']]["ts"]]
    ...
    for idx, launch in tqdm(enumerate(gpu_launch_kernels)):
        ...
        assert launch['args']['External id'] == corresp_gpu_kernel['args']['External id']
        ...
        while current_cpu_desired_index < len(cpu_desired_ops) and found_cpu == False:
            ...
            if entered_scoring == False:
                ...
                end_of_decoding_step = gpu_kernels_dict[gpu_launch_kernels[idx-1]['args']['External id']]

            # In annotation
            if cpu_start <= time_start and cpu_end >= time_end:
                name = cpu_op["name"]

                if ("ParallelEmbedding" in name or "Embedding" in name) and min_launch == None:
            ...
            # past end of annotation
                ...
                if current_cpu_desired_index < len(cpu_desired_ops) and (cpu_op["name"] == "MODULE_SCORING_AG" or cpu_op["name"] == "MODULE_LOG_SOFTMAX_AG"):
    ...
    if cpu_op["name"] == "MODULE_SCORING_AG" or cpu_op["name"] == "MODULE_LOG_SOFTMAX_AG":
...
def prep_graph(nested=False):
    ...
    fig_height = 0.35 * fig_width if args.multigpu and args.batch_size else (0.45 * fig_width if not nested else 0.8 * fig_width)
    
    fig, ax = plt.subplots(1, figsize=(fig_width, fig_height), dpi=dpi, layout='tight')    # List of labels
    return fig, ax, dpi

def wrapup_graph(plt, ax, exp_name, xlabel, yaxis_title, title, save_folder_path, filename, dpi, nested=False, file_name_passed=False):
...
def graph_gpu_kernel_breakdown(kernel_breakdown, save_folder_path):
    ...
    steps_len = len(kernel_breakdown["Scoring"]) if "Scoring" in kernel_breakdown else len(kernel_breakdown[list(kernel_breakdown.keys())[0]])
...
def graph_overall(kernel_breakdown, xlabel, exp_name, save_folder_path, nested=False, secondary_xlabel=None, file_name_passed=False):
    fig, ax, dpi = prep_graph()
    ...
    wrapup_graph(plt, ax, exp_name, xlabel, 'Execution Time Breakdown (ms)', title_name, save_folder_path, "", dpi, nested=nested, file_name_passed=file_name_passed)

def graph_overall_ratio(kernel_breakdown, xlabel, exp_name, save_folder_path, nested=False, secondary_xlabel=None, file_name_passed=False):
    fig, ax, dpi = prep_graph()
    ...
    wrapup_graph(plt, ax, exp_name, xlabel, 'Execution Time Breakdown (%)', title_name, save_folder_path, "", dpi, nested=nested, file_name_passed=file_name_passed)


def graph_overall_compare(kernel_breakdown, compare_breakdown, xlabel, exp_name, save_folder_path, file_name_passed=False):
...
def graph_overall_compare_separate_ratio(kernel_breakdown, compare_breakdown, xlabel, exp_name, save_folder_path, nested=False, file_name_passed=False):
    ...
    folder_name_split = args.json_file.split("/")
    title_name = folder_name_split[-1] + " " + exp_name + " Exp\nOperator Breakdown w/ GPU Idle(Inference)\n" + ("Warmup with 5 examples, profile result for 6th inference sample" if "t2i" in args.json_file else "Warmup with 10 examples, profile result for 11th inference sample")
    wrapup_graph(plt, ax, "\n\n"+exp_name, xlabel, 'Execution Time Breakdown (%)', title_name, save_folder_path, "_decoding_step_operator_breakdown_overall.pdf", dpi, nested=nested, file_name_passed=file_name_passed)



def graph_overall_ratio_compare(kernel_breakdown, compare_breakdown, xlabel, exp_name, save_folder_path, file_name_passed=False):
    fig, ax, dpi = prep_graph(nested=True)
    ...
    folder_name_split = args.json_file.split("/")
    title_name = folder_name_split[-1] + " " + exp_name + " Exp\nOperator Breakdown w/ GPU Idle(Inference)\n" + ("Warmup with 5 examples, profile result for 6th inference sample" if "t2i" in args.json_file else "Warmup with 10 examples, profile result for 11th inference sample")
    wrapup_graph(plt, ax, exp_name, xlabel, 'Execution Time Breakdown (%)', title_name, save_folder_path, "_decoding_step_operator_breakdown_overall_ratio.pdf", dpi, nested=True, file_name_passed=file_name_passed)
...
def graph_overall_grouped(kernel_breakdown, xlabel, exp_name, save_folder_path, secondary_xlabel):
    ...
    if args.simplify:
        ...
        for k,v in kernel_breakdown.items():
            if k == "Copy" or k == "Idle":
                ...
                new_kernel_breakdown["Communication"] = kernel_breakdown["Communication"]
            else:
                if len(new_list)==0:
                    new_list = v
                else:
    ...
    for idx, (k, v) in enumerate(kernel_breakdown.items()):
        ...
        for idx2, bs in enumerate(xlabel):
            ...
            ax.bar([xx-shift*(1-idx2) if idx2<=1 else xx+shift*(idx2-1) for xx in x], v[idx2::len(xlabel)] if len(kernel_breakdown) != 0 else [0]*len(x), bottom=bottom[idx2], label=k if idx2==0 else None, color=cmap[k], width=shift)
    ...
    plt.xticks([val for pair in zip([xx-shift*1 for xx in x], [xx-shift*0 for xx in x], [xx+shift*1 for xx in x], [xx+shift*2 for xx in x]) for val in pair], xlabel*len(secondary_xlabel), fontsize=6)
...
if args.batch_size and not args.multigpu:
                ...
                for g in range(int(ngpu)*int(nnode)):
                    if "txt_to_img" in args.json_file:
                        ...
                        for i in range(128,1024,64):
                            ...
                            file_path = '/'.join(file_dir.split("/")[:-1])+"/"+exp_info[0]+"ncs."+bs+"."+".".join(exp_info[1].split(".")[1:])+"/profile_sample_"+str(sample_id)+"_"+str(i)+"_gpu_"+str(g)+".json"
                            ...
                            gather_result({k:[vv/int(bs) for vv in v] for k, v in profile_result.items()}, _sample_multigpu_latency_breakdown)

                        if all_exist:
                            _sample_multigpu_breakdown = {k: [np.sum(v)] for k, v in _sample_multigpu_breakdown.items()}
                            _sample_multigpu_latency_breakdown = {k: [np.sum(v)] for k, v in _sample_multigpu_latency_breakdown.items()}
                        else:
                            _sample_multigpu_breakdown = dict()
                            _sample_multigpu_latency_breakdown = dict()

                        gather_result(_sample_multigpu_breakdown, sample_multigpu_breakdown)
                        gather_result(_sample_multigpu_latency_breakdown, sample_multigpu_latency_breakdown)
    ...
    folder_name_split = args.json_file.split("/")
    exp_info = folder_name_split[-1].split("ncs" if "txt_to_img" in args.json_file else "mbs")
    save_folder_path = args.graph_path+"/"+ ("wo_efficient_attn/" if "wo_efficient_attn" in args.json_file else "") + ("retrieval/" if "retrieval" in args.json_file else "batch_size_overall/")+("simplify/" if args.simplify else "")+exp_info[0]+'.'.join(exp_info[1].split(".")[1:])+"/"
    os.makedirs(save_folder_path, exist_ok=True)
    # graph_overall_grouped(overall_breakdown, BATCH_SIZE, "\n\nbatch_size", save_folder_path, secondary_xlabel=NGPU_NNODE)
    graph_overall_grouped(overall_breakdown, NGPU_NNODE, "\n\nBatch size", save_folder_path, secondary_xlabel=BATCH_SIZE)
    save_folder_path = args.graph_path+"/"+ ("wo_efficient_attn/" if "wo_efficient_attn" in args.json_file else "") + ("retrieval/" if "retrieval" in args.json_file else "batch_size_overall/")+("simplify/" if args.simplify else "")+exp_info[0]+'.'.join(exp_info[1].split(".")[1:])+"/latency/"
    os.makedirs(save_folder_path, exist_ok=True)
    ...
    for idx, (fp, dp) in tqdm(enumerate(zip(file_paths, desired_prefixes_list))):
        ...
        num_sample = 3 if "txt_to_img" in fp else len([path for path in glob.glob(fp+"/profile*.json")])
    ...
    for idx, (fp, dp) in enumerate(zip(file_paths, desired_prefixes_list)):
        ...
        for sample_id in range(warmup, warmup+num_sample):
            if "txt_to_img" in fp:
                ...
                _sample_breakdown = {k: [np.sum(v)] for k, v in _sample_breakdown.items()}
    ...
    graph_overall(overall_breakdown, model_name, "Workloads", "/fsx-atom/yejinlee/analysis_figures/breakdown/overall_breakdown.pdf", file_name_passed=True)
    graph_overall_ratio(overall_breakdown, model_name, "Workloads", "/fsx-atom/yejinlee/analysis_figures/breakdown/overall_breakdown_normalized.pdf", file_name_passed=True)
    ...
    for idx, (fp, dp) in enumerate(zip(file_paths, desired_prefixes_list)):
        ...
        if idx < 4:
            ...
            for k, v in prefill_overall_breakdown.items():
                if k in sample_breakdown:
                    prefill_overall_breakdown[k].append(np.average([sum(vv) for vv in sample_breakdown[k]]))
                    decode_overall_breakdown[k].append(0)
                else:
                    prefill_overall_breakdown[k].append(0)
                    decode_overall_breakdown[k].append(0)
    ...
    del_keys = list()
    for k, v in prefill_overall_breakdown.items():
        if sum(v)==0 and k in decode_overall_breakdown and sum(decode_overall_breakdown[k])==0:
            del_keys.append(k)
