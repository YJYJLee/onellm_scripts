...
if [ -d "$EXPDIR/results" ] ; then
    ...
    mkdir -p $EXPDIR
    FSD=/fsx-onellm/mingdachen/onellm-eval-data-tok python onellm_eval/run.py  --model-name $MODELNAME --tasks $TASK --max-batch-size $BATCHSIZE --dump-dir $EXPDIR --n-retrieved-docs $NRETRIEVED_DOCS --use-model-config-args --predictor-kwargs "{\"generation_mode\": \"text\"}" > $EXPDIR/stat.txt
fi
...
if [ -d "$CHECKDIR/images" ] ; then
    ...
    mkdir -p $EXPDIR
    FSD=/fsx-onellm/mingdachen/onellm-eval-data-tok python scripts/cm3v2/gen_images.py --exp_name $EXP_NAME --model-name $MODELNAME --batch-size $NUM_SAMPLE --tasks $TASK --exp_dir $EXPDIR --n-retrieved-docs $NRETRIEVED_DOCS --cfg $CFG --temp $TEMP --topp $TOPP --seed $SEED --num-cfg-samples $BATCHSIZE --generate > $EXPDIR/stat.txt
fi
