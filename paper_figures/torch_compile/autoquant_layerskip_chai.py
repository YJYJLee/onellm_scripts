...
def plot(batch_size_dict, first_axis, second_axis, third_axis):
    ...
    for k, bs in batch_size_dict.items():
        if k=="HSTU":
            baseline_wo_sdpa.append(sum([np.average(v) for v in get_latency(get_folder("HSTU-Pytorch", bs, "", sdpa=False)).values()]))
        else:
    ...
    for k, bs in batch_size_dict.items():
        if k=="HSTU":
            sdpa.append(sum([np.average(v) for v in get_latency(get_folder("HSTU-Triton", bs, "")).values()]))
        else:
    ...
    # create data 
    sdpa = [4,4,4,4,4,4,4,4,4,4,4,4,4]
    torch_compile = [9,9,9,9,9,9,9,9,9,9,9,9,9]
    num_llama_chameleon = 4
    # x = np.arange(len(baseline_wo_sdpa)) 
    x = np.arange(len(baseline_wo_sdpa)-num_llama_chameleon)
    ...
    sec2 = ax.secondary_xaxis(location=0)
    sec2.set_xticks([0.5, 3.625, 7.5, 8.5], labels=[])
    sec2.tick_params('x', length=65, width=0.8)
    ax.set_xlim(-0.75, 8.5)
    ...
    plt.legend(["LayerSkip", "CHAI"], ncol=2, loc="upper center")
