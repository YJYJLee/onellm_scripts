...
BATCH_SIZE=$1
PHASE=$2
SAVE_DIR="$HOME/roofline/humaneval/batch_size${BATCH_SIZE}/$PHASE"
mkdir -p $SAVE_DIR

PHASE=$PHASE time /usr/local/cuda-12.1/nsight-compute-2023.1.1/ncu --log-file $SAVE_DIR/profile_stat.txt --metrics $METRICS --print-summary per-kernel -o $SAVE_DIR/ncu_memory_bw_profile --nvtx $NVTX_COMMAND -f python /fsx-atom/yejinlee/2024_paper_projects/temp2/ATOM/eval_bigcode.py --checkpoint_path $CODELLAMA_34B/model.pth --model_name CodeLlama-34B --task humaneval --temperature 0.2 --top_p 0.95  --max_seq_len 512 --batch_size $BATCH_SIZE --log_dir ./logs/evals/codellama34b-baseline-humaneval-eager/ --sdpa naive --batch_size $BATCH_SIZE --compile > $SAVE_DIR/stat.txt
