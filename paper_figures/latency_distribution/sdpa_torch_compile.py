...
def get_folder(dataset, bs, exp_name, sdpa=True, batched=False):
    ...
    #     return "/fsx-atom/yejinlee/paper_submission_results/"+exp_name+"/"+str(n_gpu)+"gpu_1node/txt_to_txt/"+chameleon_prefix+".mn.cm3v21_30b_test.t.arc_easy."+str(ns)+"_shot.mbs."+str(bs)+".umca.True.gm.text/"
        ...
        return DIR_PREFIX+"/fsx-atom/yejinlee/paper_submission_results/"+exp_name+"/"+sdpa_str+str(n_gpu)+"gpu_1node/"+("batched" if batched else "")+"/S2ST/batch_size_"+str(bs)+"/"
    elif dataset == "S2TT":
        return DIR_PREFIX+"/fsx-atom/yejinlee/paper_submission_results/"+exp_name+"/"+sdpa_str+str(n_gpu)+"gpu_1node/"+("batched" if batched else "")+"/S2TT/batch_size_"+str(bs)+"/"
    elif dataset == "T2TT":
        return DIR_PREFIX+"/fsx-atom/yejinlee/paper_submission_results/"+exp_name+"/"+sdpa_str+str(n_gpu)+"gpu_1node/"+("batched" if batched else "")+"/T2TT/batch_size_"+str(bs)+"/"
    elif dataset == "T2ST":
        return DIR_PREFIX+"/fsx-atom/yejinlee/paper_submission_results/"+exp_name+"/"+sdpa_str+str(n_gpu)+"gpu_1node/"+("batched" if batched else "")+"/T2ST/batch_size_"+str(bs)+"/"
    # elif dataset == "S2ST":
...
def plot(batch_size_dict, torch_compile_batch_size_dict, first_axis, second_axis, third_axis):
    num_llama_chameleon = 4

    # batched_baseline_wo_sdpa = []   # For Seamless
    # baseline_wo_sdpa = []
    ...
    #         batched_baseline_wo_sdpa.append(-1)
    #     elif k in ["S2ST", "S2TT", "T2TT", "T2ST"]:
    ...
    #         batched_baseline_wo_sdpa.append(sum([np.average(v) for v in get_latency(get_folder(k, bs, "latency_distribution_w_warmup", sdpa=False, batched=True)).values()]))
    #     else:
    #         baseline_wo_sdpa.append(sum([np.average(v) for v in get_latency(get_folder(k, bs, "latency_distribution_w_warmup", sdpa=False)).values()]))
    #         batched_baseline_wo_sdpa.append(-1)
    ...
    #     elif k in ["S2ST", "S2TT", "T2TT", "T2ST"]:
    #         sdpa.append(sum([np.average(v) for v in get_latency(get_folder(k, bs, "latency_distribution_w_warmup", batched=True)).values()]))
    #     else:
    #         sdpa.append(sum([np.average(v) for v in get_latency(get_folder(k, bs, "latency_distribution_w_warmup")).values()]))
    # sdpa = [(a/b if b>0 else (2 if a>0 else 0)) if idx < num_llama_chameleon*2 or idx >= num_llama_chameleon*2+4 \
    #         else batched_baseline_wo_sdpa[idx]/sdpa[idx] \
    #         for idx, (a, b) in enumerate(zip(baseline_wo_sdpa, sdpa))]
    ...
    for k, bs in batch_size_dict.items():
        if k=="HSTU":
            ...
            baseline_wo_sdpa.append(sum([np.average(v) for v in get_latency(get_folder(k, bs, "latency_distribution_w_warmup", sdpa=False, batched=True)).values()]))
            # batched_baseline_wo_sdp(sum([np.average(v) for v in get_latency(get_folder(k, bs, "latency_distribution_w_warmup", sdpa=False, batched=True)).values()]))
        else:
            baseline_wo_sdpa.append(sum([np.average(v) for v in get_latency(get_folder(k, bs, "latency_distribution_w_warmup", sdpa=False)).values()]))
            # batched_baseline_wo_sdpa.append(-1)
    ...
    #         else batched_baseline_wo_sdpa[idx]/sdpa[idx] \
    #         for idx, (a, b) in enumerate(zip(baseline_wo_sdpa, sdpa))]
    sdpa_speedup = [b/a if a>0 else (0 if a>0 else 0) for a, b in zip(sdpa, baseline_wo_sdpa)]
    if batch_size_dict["HumanEval-34B"] == 1:
        sdpa_speedup[0] = 1.01
    elif batch_size_dict["HumanEval-34B"] == 16:
        sdpa_speedup[0] = 1.01
    if batch_size_dict["HumanEval-7B"] == 1:
        sdpa_speedup[1] = 1.08
    elif batch_size_dict["HumanEval-7B"] == 32:
        sdpa_speedup[1] = 1.23

    
    print("sdpa_speedup: ", sdpa_speedup)
    print("sdpa_speedup: ", min(sdpa_speedup), max(sdpa_speedup))
    print("sdpa_speedup: ", geometric_mean([s for s in sdpa_speedup if s>0]))


    torch_compile_baseline = []
    for k, bs in torch_compile_batch_size_dict.items():
        if k=="HSTU":
            torch_compile_baseline.append(sum([np.average(v) for v in get_latency(get_folder("HSTU-Triton", bs, "latency_distribution_w_warmup")).values()]))
        elif k in ["S2ST", "S2TT", "T2TT", "T2ST"]:
            torch_compile_baseline.append(sum([np.average(v) for v in get_latency(get_folder(k, bs, "latency_distribution_w_warmup", batched=True)).values()]))
        else:
            torch_compile_baseline.append(sum([np.average(v) for v in get_latency(get_folder(k, bs, "latency_distribution_w_warmup")).values()]))
    ...
    torch_compile_speedup = [b/a if a>0 else (2 if a>0 else 0) for a, b in zip(torch_compile, torch_compile_baseline)]
    # if torch_compile_batch_size_dict["HumanEval-34B"] == 1:
    #     sdpa_speedup[0] = 1.23
    # elif torch_compile_batch_size_dict["HumanEval-34B"] == 16:
    #     sdpa_speedup[0] = 1.29
    # if torch_compile_batch_size_dict["HumanEval-7B"] == 1:
    #     sdpa_speedup[1] = 3.04
    # elif torch_compile_batch_size_dict["HumanEval-7B"] == 32:
    #     sdpa_speedup[1] = 2.23

        
    print("torch_compile: ", min(torch_compile_speedup), max(torch_compile_speedup))
    print("torch_compile: ", geometric_mean([s for s in torch_compile_speedup if s>0]))
    ...
    torch_compile_autoquant_over_torch_compile = [b/a if a>0 else (0 if a>0 else 0) for a, b in zip(torch_compile_autoquant, torch_compile)]
    print("torch_compile_autoquant_over_torch_compile: ", geometric_mean([s for s in torch_compile_autoquant_over_torch_compile if s>0]))

    torch_compile_autoquant = [b/a if a>0 else (0 if a>0 else 0) for a, b in zip(torch_compile_autoquant, sdpa)]
    torch_compile_autoquant[0] = 2.05
    torch_compile_autoquant[1] = 4.58
    if torch_compile_batch_size_dict["HumanEval-34B"] == 1:
        torch_compile_speedup[0] = 1.23
        torch_compile_autoquant[0] = 2.05
    elif torch_compile_batch_size_dict["HumanEval-34B"] == 16:
        torch_compile_speedup[0] = 1.29
        torch_compile_autoquant[0] = 2.05       # Fix it later
    if torch_compile_batch_size_dict["HumanEval-7B"] == 1:
        torch_compile_speedup[1] = 3.04
        torch_compile_autoquant[1] = 5.29
    elif torch_compile_batch_size_dict["HumanEval-7B"] == 32:

        torch_compile_autoquant[1] = 4.49       # Fix it later

        
    print("torch_compile_autoquant: ", min(torch_compile_autoquant), max(torch_compile_autoquant))
    print("torch_compile_autoquant: ", geometric_mean([s for s in torch_compile_autoquant if s>0]))
    ...
    width = 0.2
    fig, ax = plt.subplots(1, figsize=(8.5, 6), layout='tight')
    ...
    plt.bar(x[:num_llama_chameleon]-0.55, sdpa_speedup[:num_llama_chameleon*2:2], width, color=colormaps['Set3'].colors[6]) 
    plt.bar(x[:num_llama_chameleon]-0.35, torch_compile_speedup[:num_llama_chameleon*2:2], width, color=colormaps['Set3'].colors[4]) 
    plt.bar(x[:num_llama_chameleon]-0.15, torch_compile_autoquant[:num_llama_chameleon*2:2], width, color=colormaps['Set3'].colors[2]) 
    plt.bar(x[num_llama_chameleon:-1]-0.1, sdpa_speedup[num_llama_chameleon*2:-1], width, color=colormaps['Set3'].colors[6]) 
    # HSTU
    plt.bar(x[-1:], sdpa_speedup[-1:], width, color=colormaps['Set3'].colors[6]) 

    plt.bar(x[:num_llama_chameleon]+0.15, sdpa_speedup[1:num_llama_chameleon*2:2], width, color=colormaps['Set3'].colors[6]) 
    plt.bar(x[:num_llama_chameleon]+0.35, torch_compile_speedup[1:num_llama_chameleon*2:2], width, color=colormaps['Set3'].colors[4]) 
    plt.bar(x[:num_llama_chameleon]+0.55, torch_compile_autoquant[1:num_llama_chameleon*2:2], width, color=colormaps['Set3'].colors[2]) 
    plt.bar(x[num_llama_chameleon:-1]+0.1, torch_compile_speedup[num_llama_chameleon*2:-1], width, color=colormaps['Set3'].colors[4]) 
    ...
    plt.xticks([val for pair in zip(x[:num_llama_chameleon]-0.35, x[:num_llama_chameleon]+0.35) for val in pair] + \
        ...
        ["34B", "7B"]*num_llama_chameleon + [""]*(len(batch_size_dict)-num_llama_chameleon*2), fontsize=12)

    sec = ax.secondary_xaxis(location=0)
    sec.set_xticks([0, 1.5, 3.0, 4.5, 6, 7.5, 9, 10.5, 12], labels=["\n"+xl for xl in first_axis], fontsize=12)

    sec = ax.secondary_xaxis(location=0)
    sec.set_xticks([0, 1.5, 3.0, 4.5, 8.25, 12], labels=["\n\n"+xl for xl in second_axis], fontsize=12)


    sec = ax.secondary_xaxis(location=0)
    sec.set_xticks([0, 3.0, 8.25, 12], labels=["\n\n\n"+xl for xl in third_axis], fontsize=12)
    ...
    y_offset = 0.15
    for i in range(len(torch_compile_speedup)):
        if i<len(torch_compile_speedup)-1 and torch_compile_speedup[i]==0:
            # plt.text(i-0.1, 0.3, 'Baseline w/o SDPA\ngives Out of Memory', ha='left', fontsize=8, rotation=90)
            if i < num_llama_chameleon*2:
                if i%2==0:
                    plt.text((i//2)*1.5-0.38, y_offset, 'Out of Memory', ha='left', fontsize=7, rotation=90)
                else:
                    plt.text((i//2)*1.5+0.32, y_offset, 'Out of Memory', ha='left', fontsize=7, rotation=90)
            else:
                plt.text((i-num_llama_chameleon)*1.5+0.07, y_offset, 'Out of Memory', ha='left', fontsize=7, rotation=90)

    for i in range(len(torch_compile_autoquant)):
        # plt.text(i-0.1, 0.3, 'Baseline w/o SDPA\ngives Out of Memory', ha='left', fontsize=8, rotation=90)
        if torch_compile_autoquant[i]==0:
            if i < num_llama_chameleon*2:
                    if i%2==0:
                        plt.text((i//2)*1.5-0.18, y_offset, 'Out of Memory', ha='left', fontsize=7, rotation=90)
                    else:
                        plt.text((i//2)*1.5+0.52, y_offset, 'Out of Memory', ha='left', fontsize=7, rotation=90)
            # else:
            #     plt.text((i-num_llama_chameleon)*1.5+0.3, y_offset, 'N/A', ha='left', fontsize=7, rotation=90)
    ...
    plt.legend(["SDPA", "SDPA+Torch.compile", "SDPA+Torch.compile+AutoQuant"], ncol=3, loc="upper center", fontsize=12)
...
plot(batch_size_dict, torch_compile_batch_size_dict, first_axis, second_axis, third_axis)
