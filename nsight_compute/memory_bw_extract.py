...
def process_report(file_path, filter_ps):
    ...
    for range_idx in range(report.num_ranges()):
        ...
        if first==False:
            first=True
            num_stat=len(current_range)

        if abs(num_stat-len(current_range)) > num_stat*0.1:
...
for profile_step in tqdm(range(0, END+1, PROFILE_GRANULARITY)):
    ...
    if os.path.isfile(file_path):
        ...
        if os.path.isfile(file_path):
            ...
            if "cfg" in file_path:
                file_path = re.sub(r'hahahatesttest', 'template', re.sub(r'topp', 'topp.', re.sub(r'temp', 'temp.', re.sub(r'cfg', 'cfg.', re.sub(r'template', 'hahahatesttest', args.profile_dir)))))+"/layer"+str(PROFILE_LAYER)+"/decoding_step"+profile_steps+"/ncu_memory_bw_profile.ncu-rep"
                if os.path.isfile(file_path):
                    mem, sm = process_report(file_path, filter_ps)
                    memory_bw_util += mem
                    sm_util += sm
                else:
                    file_path = re.sub(r'hahahatesttest', 'template', re.sub(r'topp', 'topp.', re.sub(r'temp', 'temp.', re.sub(r'cfg', 'cfg.', re.sub(r'template', 'hahahatesttest', args.profile_dir)))))+"/layer"+str(PROFILE_LAYER)+"/decoding_step_"+profile_steps+"/ncu_memory_bw_profile.ncu-rep"
                    if os.path.isfile(file_path):
                        mem, sm = process_report(file_path, filter_ps)
                        memory_bw_util += mem
                        sm_util += sm
...
def plot_graph(xaxis, yaxis, title):
    ...
    save_file_path += args.profile_dir.split("/")[-1]+".pdf"
